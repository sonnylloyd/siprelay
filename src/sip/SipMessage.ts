...
export class SipMessage {
  ...
  public getTargetHost(): string | undefined {
    const regex = /^\s*(?:INVITE|REGISTER|ACK|BYE|CANCEL|OPTIONS|INFO|MESSAGE|SUBSCRIBE|NOTIFY)\s+sip:[^@]+@([^>\s;]+)/i;
    const match = this.startLine.match(regex);
    return match?.[1];
  }
